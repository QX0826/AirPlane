...
public class GameLevel1 {
    ...
    private void initHeroPlane() {
        heroPlane = new HeroPlane(root, scene, level1Map.actualWidth, this::initEnemySpawn);
        heroPlane.in();
    }

    private void initEnemySpawn() {
        enemySpawn = new EnemySpawn(root, level1Map.actualWidth, () -> {
            //分数标签
            GameScore.addScore();
            ...
            if (GameScore.score >= 5) {
                ...
                heroPlane.out((() -> {
                    ...
                    //播放视频 进入第二关
                    new ReVideoView(primarystage, Assets.VIDEOS.get("movie_story_2"), () -> {

                        //进入第二关
                        new GameLevel2(primarystage);
                    });
                }));
            }
        });
